...
public class DiscardHandler extends BaseMessageHandler {
  ...
  public void handle(Session session, JsonObject msg) {
    ...
    if (!Room.PHASE_REVEAL.equals(room.getPhase())) {
      logger.warn(
          "Discard attempt in wrong phase: {} for room {}", room.getPhase(), room.getRoomId());
      return;
    }

    if (player.hasUsedKey(cardKey)) {
      logger.warn(
          "Player {} already used card {} in room {}", player.getName(), cardKey, room.getRoomId());
      return;
    }

    if (room.getRoundReveals().containsKey(player.getId())) {
      logger.warn(
          "Player {} already discarded a card this round in room {}",
          player.getName(),
          room.getRoomId());
      return;
    }
